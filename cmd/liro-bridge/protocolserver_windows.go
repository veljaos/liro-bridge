...
// The protocol server's half of the flow: what turns an HTTP request
// into the same window a person drives by hand.
//
// It is split from protocolflow.go because that file holds two things
// with different reaches. The remote *batch* — what a request looks
// like to the window, and the methods the window's own loop calls on it
// — is part of the window and compiles wherever the window does. The
// driver below is reached only from `tray`, which is the agent mode,
// and the agent mode is F12 §6's decision on this platform: a stock
// GNOME desktop has no tray, and what the agent does there is not yet
// decided. Until it is, this half has no caller on Linux, and the
// project's own rule for code whose callers are one platform's is the
// filename suffix rather than a lint exception.
