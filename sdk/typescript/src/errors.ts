 ...
 */
export type KnownLiroErrorCode =
...
const KNOWN_CODES: ReadonlySet<string> = new Set<KnownLiroErrorCode>([
  'REQUEST_INVALID',
  'NOT_PAIRED',
  'AUTH_FAILED',
  'PAIRING_CODE_INCORRECT',
...
export function isKnownAgentCode(code: string): code is KnownLiroErrorCode {
  return KNOWN_CODES.has(code);
}

/** Every agent code, for tests and for tooling that enumerates them. */
export function allKnownAgentCodes(): KnownLiroErrorCode[] {
  return [...KNOWN_CODES] as KnownLiroErrorCode[];
}
 ...
 * Ministry.
 */
const MESSAGES: Record<LiroErrorCode, string> = {
  // ---- pairing and authentication ----
  REQUEST_INVALID:
    'The agent refused the request as malformed. `details.field` names the field, or `details.maxBytes` the limit.',
  NOT_PAIRED:
    'This application is not paired with the agent, or the person disconnected it in the agent’s Settings window. Pair again.',
  AUTH_FAILED:
    'The request did not authenticate, and the agent deliberately does not say which check failed. ' +
  ...
  PAIRING_CODE_INCORRECT:
    'That six-digit code is wrong. The pairing request is still live; `details.attemptsRemaining` says how many tries are left.',
  PAIRING_DENIED:
    'The person refused the pairing, or closed the window. This is an answer, not a failure — do not ask again automatically.',
  PAIRING_ORIGIN_MISMATCH:
    'The `origin` sent with pair/confirm is not the one pair/request declared. Both calls must carry the same string, byte for byte.',
  PAIRING_IN_PROGRESS: 'Another application’s pairing window is already open on this machine. Wait and try again.',
  PAIRING_EXPIRED:
    'The pairing request is gone: five minutes passed, five wrong codes voided it, it was already confirmed, or there is no such request. Start a new one.',
  RATE_LIMITED:
    'Too many pairing requests from this origin. `details.retryAfterSeconds` says how long to wait.',

  // ---- jobs ----
  DOCUMENT_SIGNING_DISABLED:
    'This machine is set up to serve only POST /v2/sign; the whole-document path is switched off. ' +
  ...
  CERTIFICATE_LISTING_DISABLED:
    'The certificate listing is switched off on this machine. That is a setting in the agent’s Settings window, not a fault in your request — ' +
  ...
  JOB_NOT_FOUND:
    'No such job: it never existed, it belongs to another application, its result has already been collected, or it expired. Submit again.',
  ENDPOINT_NOT_FOUND:
    'This agent has no such endpoint. Your request is almost certainly right and the agent is older than this SDK — ' +
  ...
  SMART_CARD_SERVICE_DOWN:
    'The operating system’s smart card service is not running. That is a service to start, not hardware to plug in.',
  ...
  PIN_INCORRECT:
    'The PIN was wrong. NEVER retry this automatically: three wrong entries block the card, and unblocking a national identity card means a visit to the Ministry. Show the person what happened and let them decide.',
  PIN_LOCKED: 'The card is blocked and needs its PUK. Stop.',
  CERT_NOT_FOUND:
    'No certificate with that thumbprint is available on this machine. Check the thumbprint against GET /v2/certificates, or leave it out on /v2/sign/pdf.',
  ...
  TSA_CLIENT_CERT_UNREADABLE:
    'The TSA client certificate configured on this machine could not be read. That is the person’s Settings, not your request.',
  TSA_CLIENT_CERT_INVALID:
    'The TSA client certificate would not open — almost always a wrong password. That is the person’s Settings, not your request.',
  STAMP_GLYPH_MISSING:
    'The visible stamp needs a character the embedded font does not have. `details.character` and `details.codePoint` name it.',
  ...
  OUTPUT_WRITE_FAILED:
    'About writing a file. This happens to batches the person started locally; a request over the protocol writes nothing.',
  ...
  // ---- raised by this SDK ----
  AGENT_NOT_RUNNING:
    'Liro Bridge is not running, or its discovery file could not be read. Start the agent and try again; never scan ports for it.',
  ...
  UNKNOWN:
    'The agent answered with a code this version of the SDK does not know. `agentCode` carries it verbatim; the agent is probably newer than this SDK.',
};

/** The English developer-facing sentence for a code. */
export function messageForCode(code: LiroErrorCode): string {
...
export class LiroError extends Error {
  ...
  static fromAgent(
    ...
    const options: LiroErrorOptions = {
      details,
      httpStatus,
      agentCode: rawCode,
