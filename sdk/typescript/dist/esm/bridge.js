...
export class LiroBridge {
    ...
    static async connect(options) {
        ...
        if (info.protocolVersion !== 0 && info.protocolVersion !== PROTOCOL_VERSION) {
            throw new LiroError('VERSION_TOO_OLD', {
                detail: `the agent speaks protocol version ${info.protocolVersion} and this SDK speaks ${PROTOCOL_VERSION}. ` +
    ...
    async certificates() {
        ...
        const body = parseJSON(response, 'GET /v2/certificates');
    ...
    async signPdf(documents, options = {}) {
        ...
        return this.toSignedDocuments(parseJSON(result, job.resultUrl), normalised, job);
    }
    /**
    ...
    async signDigests(digests, options) {
        ...
        const result = await this.awaitResult(job, options.signal, options.timeoutMs);
        return this.toSignatures(parseJSON(result, job.resultUrl), digests.length, job);
    }
    /**
    ...
    async echo(body = new Uint8Array(0)) {
        ...
        const parsed = parseJSON(response, 'POST /v2/echo');
    ...
    async submit(path, body, expected, signal, timeoutMs) {
        ...
        const parsed = parseJSON(response, `POST ${path}`);
    ...
    async collectResult(job, signal, deadline) {
        for (;;) {
            ...
            this.emit(job.jobId, parseJSON(response, job.resultUrl));
...
async function fetchHealth(transport) {
    ...
    const body = parseJSON(response, 'GET /v2/health');
    return {
