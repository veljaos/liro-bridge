...
exports.FileSecretStore = exports.StoredPairing = exports.REDACTED_SECRET = void 0;
 ...
 */
exports.REDACTED_SECRET = '[redacted: persist pairing.serialise(), not JSON.stringify(pairing)]';
...
class StoredPairing {
    ...
    deviceSecretBytes() {
        ...
        if (bytes === undefined) {
            throw new errors_js_1.LiroError('SECRET_STORE_INVALID', {
    ...
    static deserialise(record) {
        if (typeof record !== 'object' || record === null) {
            throw new errors_js_1.LiroError('SECRET_STORE_INVALID', { detail: 'the stored pairing is not an object' });
        }
        const r = record;
        const appId = r['appId'];
        ...
        if (deviceSecret === exports.REDACTED_SECRET) {
            throw new errors_js_1.LiroError('SECRET_STORE_INVALID', {
    ...
    toJSON() {
        return {
            ...
            applicationName: this.applicationName,
            origin: this.origin,
            deviceSecret: exports.REDACTED_SECRET,
        };
    }
...
exports.StoredPairing = StoredPairing;
...
class FileSecretStore {
    ...
    async read() {
        let text;
        try {
            text = await (0, promises_1.readFile)(this.path, 'utf8');
        }
        catch (err) {
            if (err.code === 'ENOENT') {
    ...
    async write(file) {
        ...
        const temporary = (0, node_path_1.join)(directory, `.liro-secrets-${(0, node_crypto_1.randomBytes)(8).toString('hex')}.tmp`);
        const body = `${JSON.stringify(file, null, 2)}\n`;
        try {
            await (0, promises_1.writeFile)(temporary, body, { encoding: 'utf8', mode: 0o600 });
            await restrictToCurrentUser(temporary);
            await (0, promises_1.rename)(temporary, this.path);
        }
        catch (err) {
...
async function restrictToCurrentUser(path) {
    ...
    await new Promise((resolve, reject) => {
        (0, node_child_process_1.execFile)('icacls', [
            path,
            '/inheritance:r',
            '/grant:r',
            `${user}:(F)`,
            ...
            if (err) {
                reject(new errors_js_1.LiroError('SECRET_STORE_INVALID', {
...
function windowsAccountName() {
    ...
    const name = process.env['USERNAME'] ?? (0, node_os_1.userInfo)().username;
