...
exports.assertUsableEnvironment = assertUsableEnvironment;
const errors_js_1 = require("./errors.js");
/** The oldest Node this SDK supports. */
exports.MINIMUM_NODE_MAJOR = 18;
/**
...
function assertNotBrowser() {
    ...
    if (hasDocument || hasWindow || isDenoOrWorkerd) {
        throw new errors_js_1.LiroError('UNSUPPORTED_ENVIRONMENT', {
            detail: 'this SDK is running in a browser or a browser-like runtime. A device secret in a page is a secret ' +
...
function assertSupportedNode() {
    ...
    if (!Number.isFinite(major) || major < exports.MINIMUM_NODE_MAJOR) {
        throw new errors_js_1.LiroError('UNSUPPORTED_ENVIRONMENT', {
            detail: `this SDK needs Node ${exports.MINIMUM_NODE_MAJOR} or later and is running on ${node}`,
        });
    }
    if (typeof globalThis.fetch !== 'function') {
        throw new errors_js_1.LiroError('UNSUPPORTED_ENVIRONMENT', {
            detail: `Node ${node} has no global fetch. Run with Node ${exports.MINIMUM_NODE_MAJOR} or later, and without --no-experimental-fetch`,
        });
    }
}
/** Both checks, in the order that produces the more useful message first. */
function assertUsableEnvironment() {
    assertNotBrowser();
    assertSupportedNode();
}
