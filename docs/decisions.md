...
**The pair now embedded is the third generation, 2026-09-11.** Folded
into this entry rather than opened as a new one because it is the same
decision arriving at its own values — the same shape D-221 used when its
own gap was closed. Recorded because two earlier generations exist in
this repository's history and in terminal output, and a public key found
in a log months from now is worth nothing unless something says whether
it was ever trusted. **None of the first two ever was.**

| Generation | What it was | Why it went |
|---|---|---|
| 1 | A pair, generated inside an agent session, committed to `internal/update/trustedkeys.go` at `1d048fa` | Its provenance rested on a claim. The reasoning is the body of this entry: a key that can publish an update every installed agent accepts without argument is not one whose cleanliness should rest on the agent's own account of itself. |
| 2 | **Two single keys, not a pair at all** — `scripts/genreleasekey`'s `--pair` flag was lost to a line break in PowerShell, so each of two runs produced one key | Never a usable pair, never committed, never embedded. Both files deleted. |
| 3 | The pair in use, generated by the owner on his own machine | — |

The first generation's public halves are
`DfVtjTl9lKSjZQB07a4dB6OcRIQPAuNGk0R9VDspvCk=` (primary) and
`BNiw9ruQH5nNAJdNyzeOygTZCytQxVstE3g0gP1z5BE=` (spare). They are public
halves, so naming them costs nothing, and naming them is what lets the
check below exist. **No release was ever signed with either**: the
`release` environment did not hold a key while they were in the file,
and no tag has been pushed.

The second generation's public halves are deliberately **not** recorded,
because this session never saw them. That is a gap in the list rather
than an omission from it, and it is said here so that a reader meeting
one of those two values does not take this table for exhaustive. What
can be said about them is what matters: neither was ever a pair, neither
was committed, and nothing has ever been signed with either.

`TestNoDiscardedReleaseKeyIsTrustedByThisBuild` turns the first
generation's half of that from a sentence into a check — it fails if
either of those two values is ever embedded again, which is the one way
a revert of `trustedkeys.go` could quietly undo this. Confirmed to fire:
with the first-generation primary pasted back it reports "embedded key 0
is the first-generation primary, which was discarded and must never be
trusted". The second generation cannot be checked for, for the reason
above.

**A rule this leaves behind, because the mechanism is general.** The
body of this entry records that for a secret, the agent's own output
channel is a place of record — "do not write it to a file" and "do not
let it through the agent" are different requirements. Generation 2 is
the same sentence pointed at a *tool*: a command whose flag is lost in
transit fails by producing something plausible rather than by refusing.
`genreleasekey` without `--pair` does not err; it writes one key, which
is a correct answer to a question nobody asked. Whoever next runs it
should read back the file it wrote before pasting anything, rather than
reading back the command they believe they typed.
