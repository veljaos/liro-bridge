...
- **The PIN exists only for the duration of `C_Login`,** and is overwritten immediately afterwards. It is not left for the garbage collector, not held in a struct field, not captured by a closure that outlives the call, and not merely dropped.
  ...
  **It crosses exactly one process boundary, and that boundary is the second place in its path that this program does not own.** A vendor PKCS#11 module runs in a worker process of this program's own, because a module that kills its host must not take the agent, its windows and a batch in flight with it — measured, on a real module, and not survivable in-process. `C_Sign` requires a logged-in session and a session belongs to the process that opened it, so `C_Login` happens in the worker while the screen that collects the PIN belongs to the agent. The PIN is therefore written once, by the agent, into a pipe the agent created and the worker inherited.

  **What that costs, named exactly rather than minimised.** Between the two ends is a kernel pipe buffer. **It is not this program's memory, this program cannot wipe it, and its lifetime is the kernel's rather than ours.** It is freed when it has been read and the pipe is closed, and nothing this program does can make that happen sooner or prove that it happened. That is a real exposure and this clause does not pretend otherwise. It is permitted because it is a different object from the one §10 refuses for the PIN screen — there, the PIN would live in a DOM node, a script engine's heap and an inter-process message inside a browser process this program neither owns nor can reach, collected on a schedule that is not ours. Here both ends are ours, both are wiped, and the middle is the kernel's and is freed on read. The reasoning that refuses the first does not automatically refuse the second, and this clause says which is which rather than leaving it to be re-argued.

  **What bounds it, and every one of these is a requirement.** One write of the PIN's bytes and no more. Read immediately and in full by the worker. **Never buffered by this program on either side** — not through a buffered writer, not through a buffered reader, not through a queue, not through anything that would hold a copy after the call that put it there has returned. **Never an environment variable, never a command-line argument, never a file, and never a socket anything else on the machine can connect to.** Each of those is readable by another process running as the same user by ordinary means; an inherited pipe is not, and that difference is the whole reason this is the only boundary permitted.

  **What is unchanged, which is everything else.** Both ends are this program's own memory, are pinned, and are overwritten **through the pinned address** rather than through a slice header, because a loop that was elided and a loop that ran look identical through the header. No struct field, no closure outliving the call. Nothing retries a PIN, ever, for any reason. `ulMinPinLen` and `ulMaxPinLen` are read from the token and enforced by this layer *before* the write, so an invalid length never reaches the pipe and never reaches the card. The screen that collects it says it is Liro Bridge asking. And the AST guard that makes all of this a property of the code rather than of anybody's care **extends to the worker's own package**, which is new and must carry one from its first commit rather than acquire one afterwards.

  **This is the second exception to a clause written absolutely, and it is said here rather than left to be counted.** The first is the native dialog's own edit control, whose copy of the PIN is the operating system's memory: it cannot be wiped, only overwritten through the control's own interface, and it is, before the window is destroyed rather than by destroying it. **A third should be suspected of being a pattern rather than a case** — at which point this clause has stopped describing what the program does, and the honest move is to rewrite it rather than to add to it again.
- **It is never written anywhere by this program:** not to a log, not into an error, not into a report, not into the audit log, not to a file. That is a property of this program's own code, and clause 2's guard is what keeps it one.

  **What this program cannot promise, said rather than implied.** While the PIN is in memory — one `C_Login`, bracketed by the wipe — the operating system may capture that memory in a crash dump, and nothing available to this program prevents it. Measured: `WerRegisterExcludedMemoryBlock` returns success and does not exclude the buffer from a full-memory dump; a per-process error-reporting disable returns success and cannot be shown to change what a report collects; and `CryptProtectMemory` does keep plaintext out of a dump but covers about 10 µs of a window that is the length of a card operation, which is under one per cent of it.

  **And the copy that matters was never this program's.** `C_Login` takes the PIN *by pointer*. The module reads it and may keep its own copy, in memory this program does not own, cannot reach and cannot wipe, for as long as it chooses. **That is not a limit of the mechanisms tried; it is a limit of handing a secret to somebody else's code**, and no protection of this program's own buffer can change it. Anyone reaching for something that covers more of the window should start here, because there is nothing to find.

  **So the promise is bounded, and the bound is the window.** The PIN is plaintext for the length of one `C_Login` and no longer, in a process that holds nothing else of value, on a machine where §6.5 already concedes that anything running as the same user can read it live. This clause forbids this program from *recording* the PIN. It does not claim the operating system cannot.
...
**What deferring F13 costs is smaller than it looked, and the reason is a machine nobody has to own.** GitHub Actions offers macOS runners, and they are free on public repositories, which this one is. So most of F13 can be built the way every phase of this project has been built — with the machine in hand: compilation and cross-compilation, the Keychain logic of §6.4, the PKCS#11 layer of §11.11 against a software token such as softhsm, the packaging, and even opening a window and photographing it. What genuinely needs hardware on a desk is three things and they are the same three the exit condition already names: **a person clicking Approve** — §6.5's only real gate, which D-094 forbids being simulated — **a card and a PIN**, and **how Gatekeeper meets an unsigned program** (§15.1). This does not undefer the phase: it says that when it is taken up, most of it is measurable rather than reasoned, and that the part that is not is small and known in advance.

**And the Mac days will be days of finding, not confirming.** Everything Linux found had built and passed its tests first: a window host whose binding generates every asynchronous operation's finishing half and none of its starting halves, so the whole of Go→page was unreachable through it; a doc comment that described the opposite of what its own function sent, which the second host was then written to believe; pages that had drifted from their own documented design; and three places where the platform cannot do what the Windows contract says. A green suite on a platform this program has never run on is evidence that it compiles there, and it is not evidence of anything else.
